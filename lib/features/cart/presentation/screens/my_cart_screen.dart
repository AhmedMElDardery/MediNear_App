...
class MyCartScreen extends StatelessWidget {
    ...
    return Consumer<CartProvider>(
      builder: (context, provider, child) {
        ...
        return Scaffold(
          ...
          appBar: AppBar(
            title: Text(pharmacyName, style: TextStyle(color: textColor, fontWeight: FontWeight.bold)),
            centerTitle: true,
  ...
  Widget _buildBottomSummary(BuildContext context, double total, List<dynamic> items, Color cardColor, Color? textColor) {
    return Container(
      padding: const EdgeInsets.all(20),
      ...
      child: Column(
        children: [
          Row(
            ...
            children: [
              const Text("Total", style: TextStyle(color: AppColors.primaryLight, fontWeight: FontWeight.bold, fontSize: 18)),
              Text("${total.toInt()} EGP", style: TextStyle(color: textColor, fontWeight: FontWeight.bold, fontSize: 18)),
            ],
          ),
              ...
              onPressed: () {
                Navigator.push(
                  ...
                  MaterialPageRoute(builder: (context) => CheckoutScreen(
                    subtotal: total,
                    pharmacyItems: List.from(items), //  بنبعت منتجات الصيدلية دي بس
                    pharmacyName: pharmacyName,      //  وبنبعت الاسم عشان نمسحهم
                  )),
                );
              },
