...
import 'package:provider/provider.dart'; // 🚀 للتعامل مع الـ Provider
...
class _MyOrdersScreenState extends State<MyOrdersScreen> {
  ...
  List<OrderModel> _getFilteredOrders(List<OrderModel> allOrders) {
    return allOrders.where((order) {
      final matchesSearch = order.pharmacyName.toLowerCase().contains(_searchText.toLowerCase()) || 
                           order.id.contains(_searchText);
      final matchesStatus = _selectedStatus == "All" || order.status == _selectedStatus;
      final matchesPharmacy = _selectedPharmacy == "All" || order.pharmacyName == _selectedPharmacy;
                ...
                Row(
                  children: [
                    ...
                    Expanded(child: _buildFilterMenu(
                      ...
                      items: ["All", "MediNear", "El-Ezaby", "Seif Pharmacy"], 
                      onSelected: (val) => setState(() => _selectedPharmacy = val)
                    )),
                  ],
                ),
  ...
  Widget _buildFilterMenu({required String title, required String currentValue, required List<String> items, required Function(String) onSelected}) {
        ...
        child: Column(
          ...
          children: [
            ...
            Row(
              ...
              children: [
                Expanded(child: Text(currentValue, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.bold, fontSize: 13), overflow: TextOverflow.ellipsis)),
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          Text("No orders found!", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: textColor))
        ]
      )
    );
  }
